...
window.addEventListener('DOMContentLoaded', function () { //todo add getter with settings
    ...
    setTimeout(async function () {
        settings = await getter("https://re.jitsi-mathu.de/getKonfSettings")
        getter("https://re.jitsi-mathu.de/getActiveMeetings?meetingID="+sessionStorage.getItem("BBB_meetingID")+"&name="+sessionStorage.getItem("BBB_fullname")).then((val)=>{
            console.log(val)
            if (!val.running&&val.visitor) {
                getter("https://re.jitsi-mathu.de/deleteActiveMeeting?meetingID="+sessionStorage.getItem("BBB_meetingID"))
                index= 1
